    ...
    <section id="promo" class="py-16 bg-white" data-aos="fade-up">
        <div class="container mx-auto px-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" data-aos="fade-right" data-aos-delay="100">
                ...
                @if (isset($promo->product) && $promo->status === 'active') <!-- Check if the promo is active -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                @foreach ($products as $product)
                    @php
                        ...
                        $isPromoActive = $promo && $promo->status === 'active' && (now()->lt(\Carbon\Carbon::parse($promo->expiry_date)));
                    @endphp
                            ...
                            <div class="flex justify-between items-center">
                                @if ($isPromoActive)
                                    @php
                                        ...
                                        $discountedPrice = $promo->discount_type === 'percentage'
                                            ? $originalPrice - $originalPrice * ($promo->discount_value / 100)
                                            : max($originalPrice - $promo->discount_value, 0);
                                    @endphp
                                    <span class="text-gray-400 line-through text-sm">Rp {{ number_format($originalPrice, 0, ',', '.') }}</span>
                                    <span class="text-primary font-bold">Rp {{ number_format($discountedPrice, 0, ',', '.') }}</span>
                                @else
